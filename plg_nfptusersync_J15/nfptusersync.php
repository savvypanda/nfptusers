...
class plgSystemnfptusersync extends JPlugin {
	...
	function onAfterInitialise() {
		...
		if(!empty($updateusers)) {
			...
			$defaultUserGroup = $users_config->get('new_usertype','Registered');
			$acl = JFactory::getACL();
			$defaultGroupId = $acl->get_group_id('',$defaultUserGroup,'ARO');
			...
			foreach($updateusers as $nfpt_user) {
				...
				if($nfpt_user->deleted) {
					if(!$user) {
						...
						//necessary helper variable for preventing the nfptuserchange plugin from firing when we save the user
						$nfptuserpluginflag = true;
						if($user->delete()) {
							$success = true;
						} else {
							$this->addlog($nfpt_user->email, 'SYNC', 'ERROR', 'Could not delete user. Please contact tech@mangotreemedia.com for assistance.');
						}
						$nfptuserpluginflag = false;
					}
				//***************************************
	...
	private function db_connect() {
		...
		if($this->nfpt_db instanceof JException) {
			return $this->nfpt_db = false;
		}
...
HEREDOC;
			...
			if(!$mailer->Send()) {
				throw new Exception('There is an error in the user application. Please contact tech@mangotreemedia.com if you continue to experience this issue.');
			}
		}
	}
