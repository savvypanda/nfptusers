...
class plgUsernfptuserchange extends JPlugin {
	...
	function onBeforeStoreUser($olduser, $isnew) {
		...
		if(isset($nfptuserpluginflag) && $nfptuserpluginflag === true) {
			return true;
		}

		//if we cannot connect to the database we can't continue
		if(!$this->db_connect($olduser['email'])) {
			JFactory::getApplication()->redirect(JRequest::getString('HTTP_REFERRER', '', 'SERVER'), 'Unable to connect to users database. Please try again later, or contact the website administrator for assistance.');
			return false;
		}

		//we need to have both the old user information and the new user information available
		$this->olduserinfo = array();
		$newemail = $olduser['email'];
		$newusername = $olduser['username'];
		if(($isnew && isset($_POST['email'])) || (isset($_POST['id']) && $_POST['id'] == $olduser['id'] && isset($_POST['email']))) {
			$newemail = $_POST['email'];
		}
		if(($isnew && isset($_POST['username'])) || (isset($_POST['id']) && $_POST['id'] == $olduser['id'] && isset($_POST['username']))) {
		...
		if($isnew || $newemail != $olduser['email']) {
			$query = 'SELECT count(*) FROM users_live WHERE email='.$this->nfpt_db->quote($newemail);
			$this->nfpt_db->setQuery($query);
			$this->nfpt_db->query();
			if($this->nfpt_db->loadResult() != 0) {
				JFactory::getApplication()->redirect(JRequest::getString('HTTP_REFERRER', '', 'SERVER'), 'A user with that email address already exists in our user database. Please use a different email address or contact an administrator for assistance.');
				return false;
			}
		}
		//we also don't want to insert duplicate usernames since those are supposed to be unique
		if($isnew || $newusername != $olduser['username']) {
			$query = 'SELECT count(*) FROM users_live WHERE username='.$this->nfpt_db->quote($newusername);
			$this->nfpt_db->setQuery($query);
			$this->nfpt_db->query();
			if($this->nfpt_db->loadResult() != 0) {
				JFactory::getApplication()->redirect(JRequest::getString('HTTP_REFERRER', '', 'SERVER'), 'A user with that username address already exists in our user database. Please use a different username or contact an administrator for assistance.');
				return false;
			}
		}
	 ...
	 */
	function onAfterStoreUser($user, $isnew, $success, $msg) { //J1.5 method name
		global $nfptuserpluginflag;
	...
	private function db_connect($emailval) {
		...
		if($this->nfpt_db instanceof JException || $this->nfpt_db->getErrorNum()) {
			$this->nfpt_db = false;
...
HEREDOC;
			$mailer->addRecipient($this->params->get('failemail'));
			$mailer->setSender($config->getValue('config.mailfrom'), $config->getValue('config.fromname'));
			$mailer->setSubject('User Save Failure');
			$mailer->setBody($message);
			if(!$mailer->Send()) {
				JError::raiseError(500, 'Unable to connect to user database. Please contact tech@mangotreemedia.com for assistance.');
			}
...
HEREDOC;
			$mailer->addRecipient($this->params->get('failemail'));
			$mailer->setSender($config->getValue('config.mailfrom'), $config->getValue('config.fromname'));
			$mailer->setSubject('Userlog Failure');
			$mailer->setBody($message);
			if(!$mailer->Send()) {
				JError::raiseWarning(400,'Unable to save information to our other sites. Please contact tech@mangotreemedia.com for assistance.');
			}
		}
	}
}
